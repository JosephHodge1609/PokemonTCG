...
module.exports = {
    ...
    case 'Open':
                    ...
                    for(let i = 0; i <= gen1cardscommon.length; i++){
                        CollectedCards.push(gen1cardsuncommon[i]);
                        CollectedCardsattachment.push(gen1cardsuncommonattachment[i]);
                    }
                    ...
                    collector2.on('collect', async i => {
                         if(i.customId === 'previous'){
                            ...
                            .setImage(CollectedCardsattachment[PositionCounter]);
                            await i.deferUpdate();
                            await i.editReply({ephermal: true, content: "Going back I see", embeds: [newEmbed], files: [attachment], components: [ViewingCards]});
                        }
                        else if(i.customId === 'next'){
                            if(PositionCounter < CollectedCards.length ){
                                PositionCounter++;
                            }
